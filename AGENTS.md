...
- Root mutation outside the default mutable area must be covered by a
  `PROJECTION_ROOT_OPERATION` artifact, or by explicit root surgery.
- Capsule Base seed authoring requires the `base-seed-authoring` operation:
  target root and actor root must both be Capsule Base, and the write set must
  stay inside the seed-owned paths declared by
  `projection-root/policy/seed-manifest.json`.
- Treat `AGENTS.md`, `PROJECTION.md`, `README.md`, and `projection-root/` as
  sealed root material during ordinary work unless an operation artifact covers
  the exact write set.
- Editing core, origin, nucleus, body-ref,
  ...
  outside a valid Base authoring operation requires an explicit root surgery or
...
```txt
id | obligation | scope
agents:base-authoring-seed-window | Capsule Base may author only seed-owned system-layer paths declared by the seed manifest through a base-seed-authoring operation artifact without opening projection-owned root material. | agents,projection-policy,root-docs
...
agents:default-mutable-area | Ordinary implementation, documentation, tooling, and proof work stays within the current body or projection-support; root documents and projection-root changes require an operation artifact, explicit root surgery, or body replacement. | agents,repository,root-docs,body,projection-support
